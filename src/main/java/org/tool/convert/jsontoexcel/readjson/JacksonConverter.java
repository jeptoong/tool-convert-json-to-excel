...
import org.tool.convert.exception.ConverterException;
import org.tool.convert.exception.ConverterUnsupportedException;
import org.tool.convert.exception.ResponseFailureException;
import org.tool.convert.utils.CommonUtils;
...
public class JacksonConverter {
    ...
    private Object readJavaType(JavaType javaType, InputStream body, String path, AtomicReference<Throwable> cause) {
        if (this.validResponseTemplateClass(ResponseTemplate.class)) {
            ...
            if ("000000".equals(responseTemplate.getResponseCode())) {
                Object result = this.getResultObject(responseTemplate, path);
                return this.convertJavaType(javaType, result);
            } else {
                cause.set(new ResponseFailureException("Error: " + responseTemplate.getResponseCode()));
            }
        } else {
    ...
    private Object getResultObject(ResponseTemplate responseTemplate, String path) {
        try {
            return CommonUtils.isEmpty(path) ? responseTemplate.getData() : propertyUtilsBean.getProperty(responseTemplate.getData(), path);
